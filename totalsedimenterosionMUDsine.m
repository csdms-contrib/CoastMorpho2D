function  [E,Eflow,B]=totalsedimenterosionMUDsine(ho,h,taucr,taucrVEG,VEG,me,kowave,MannS,fTide,U,FcrUT,FcrUTveg,hlimC,computetidalcurrent,US,hS,nSHALLOW,computeSeaWaves,Uwave_seaI,Tp_seaI,computeSwellWave,Uwave,Tp_swell,computeRiver,fMFriver,UR,flowdestroyVEG,B);
...
taucrDESTROYVEG=taucrVEG;%%%*2 maybe???
...
%Ui(VEG==1)=min(Ui(VEG==1),UTmax(VEG==1));
%Ui=min(Ui,UTmax);
Ui=Ui+UR;
%Utide=min(Utide,UTmax);
%tauC=1030*9.81*MannS.^2.*max(h,hlimC).^(-1/3).*Utide.^2; 
%tauC=1030*9.81*MannS.^2.*h.^(-1/3).*Ui.^2; 
tauC=1030*9.81*MannS.^2.*max(h,0.1).^(-1/3).*Ui.^2; 
...
%Ei=me.*(sqrt(1+(tauC./taucro).^2)-1);%E=E+1/(ncyc+1)*me.*taucr./taucro.*(sqrt(1+(tauC./taucr).^2)-1);
Ei=Ei+1/(ncyc+1)*me.*max(0,tauC-taucro)./taucro;%E=E+1/(ncyc+1)*me.*taucr./taucro.*(sqrt(1+(tauC./taucr).^2)-1);
%if flowdestroyVEG==1; B(tauC>taucrDESTROYVEG)=0; end
if flowdestroyVEG==1; B(tauC>taucrDESTROYVEG & h>0.1)=0; end
end
Eflow=Ei;
end
...
tauC=1030*9.81*MannS.^2.*max(h,0.1).^(-1/3).*UR.^2; 
Ei=me.*max(0,tauC-taucro)./taucro ;
Eflow=Ei;
if flowdestroyVEG==1; B(tauC>taucrDESTROYVEG & h>0.1)=0; end
end
...
fw=0.00251*exp(5.21*(aw/kowave).^-0.19);fw(aw/kowave<pi/2)=0.3;%fw=0.015;
...
% %tauC=1030*9.81*MannS.^2.*h.^(-1/3).*UR.*max(UR,0.5);  
