function [deltaY1,deltaY2,deltaY3,Pedge,Y2OX,EdgeERY1,EdgeERY2,EdgeERY3]=EdgeerosionBANKPUSH(P,Unx,Uny,z,aw,maxedgeheight,fox,dt,dx,MASK,A,fracY1,fracY2,fracY3,Y2OX);
...
Pedge=[P(:,1)*0 P(:,1:end-1).*(Uny(:,1:end-1)>0)]...%;%northward
     +[P(1,:)*0; P(1:end-1,:).*(Unx(1:end-1,:)>0)]...%;%;%eastward
     +[P(:,2:end).*(Uny(:,2:end)<0) P(:,end)*0]...%;%southward
     +[P(2:end,:).*(Unx(2:end,:)<0); P(end,:)*0];%westward
...
if length(aw)>1;
a=find(r<aw(edg).*Pedge(edg)*dt/dx);
else
