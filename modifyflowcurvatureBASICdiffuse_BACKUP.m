function F=diffusefetch(A,F,alpha,dx,URx,URy,MASK,Ux,Uy,h);
...
% if (k==N);UR=(URy(p(a))+URy(q(a)))/2.*min(h(p(a)),h(q(a)))./h(p(a));up=find(UR>0);Y=UR(up);end %East-west
% if (k==-N);UR=(URy(p(a))+URy(q(a)))/2.*min(h(p(a)),h(q(a)))./h(p(a));up=find(UR<0);Y=-UR(up);end
% if (k==1);UR=(URx(p(a))+URx(q(a)))/2.*min(h(p(a)),h(q(a)))./h(p(a));up=find(UR>0);Y=UR(up);end  %North-south
% if (k==-1);UR=(URx(p(a))+URx(q(a)))/2.*min(h(p(a)),h(q(a)))./h(p(a));up=find(UR<0);Y=-UR(up);end
