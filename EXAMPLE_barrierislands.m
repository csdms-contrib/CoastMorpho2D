...
P.wavefrictionCollins=0;%on/off
...
P.YUi=100002;%initial thickess of active layer
...
angleSWELLserie=NaN*ones(numberserie,1);
    ...
    %option 2
    angleSWELLserie=dirsign.*(rand(numberserie,1)*45/2);% +45/2;
...
s=0;step=0;tic;
for t=1:tmax%750%tmax;%11309:tmax;  %iteration over the tmax time stpes
...
ax2 = subplot(2,2,3);%s+1 %set(IM,'alphadata',~(A==0));s+1
...
ax4 = subplot(2,2,4);%s+1 %set(IM,'alphadata',~(A==0));s+1
