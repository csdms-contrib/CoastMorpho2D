function [E,Etide,Eswell]=totalsedimenterosionMUDsine(A,U,MANN,VEG,fTide,UR,Uwave_sea,Uwave,Tp_sea,Tp_swell,fMFswell,fMFsea,fMFriver,taucr,tcrgradeint,leveltauincrease,taucrVEG,me,h,lev,TrangeVEG,computeSeaWaves,computeSwellwave,computeRiver);
fUpeak=pi/2;
...
hoffshorelim=20;
tauWswell(h>hoffshorelim)=max(taucr*1.5,tauWswell(h>hoffshorelim));%to avoid depsotion offshore

%E=E+me*max(0,tauWswell-taucr)./taucro*fMFswell;
Eswell=me.*(sqrt(1+(tauWswell./taucro).^2)-1)*fMFswell.*fTide;
E=E+Eswell; %OCIO AI TOLTO QUIESTI fTide IN APRIL 18th 2018
end
