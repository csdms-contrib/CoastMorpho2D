...
if transx_y==1
Q=squeeze(isfinite(flyr(:,Mi,:)));Q=cumsum(Q,2);
%zi=[zb(:,Mi)  zb(:,Mi)+Yb(:,Mi) zb(:,Mi)*ones(1,nlyr)+Yb(:,Mi)*ones(1,nlyr)+Q*dlyr zb(:,Mi)+Yb(:,Mi)+plyr(:,Mi)*dlyr+Y(:,Mi) zb(:,Mi)+Yb(:,Mi)+plyr(:,Mi)*dlyr+max(0,Y(:,Mi))];ORIGNAL
zi=[zb(:,Mi)  zb(:,Mi)+Yb(:,Mi)   zb(:,Mi)*ones(1,nlyr)+Yb(:,Mi)*ones(1,nlyr)+Q*dlyr+min(0,Y(:,Mi))   zb(:,Mi)+Yb(:,Mi)+plyr(:,Mi)*dlyr+Y(:,Mi)   zb(:,Mi)+Yb(:,Mi)+plyr(:,Mi)*dlyr+Y(:,Mi)];%*ones(1,5);zi(:,2:3)=zi(:,2:3)+2.5;
